...
class CardScreenViewModelTest {
    ...
    private val addCard: AddCards = mock()
    ...
    @Before
    fun setup() {
        useCase = CardUserCases(addCardUseCase, updateCardUseCase, deleteCardUseCase, getCardUseCase)
    ...
    fun `test insertCard with valid data`() = runTest {
        ...
        whenever(useCase.addCardUseCase(validCard)).thenReturn(Unit)
        ...
        verify(useCase.addCardUseCase).invoke(validCard)
    }
    ...
    fun `test insertCard with invalid card number`() = runTest {
        ...
        // Mocking the addCardUseCase to do nothing
        whenever(useCase.addCardUseCase(invalidCard)).thenReturn(Unit)
        ...
        verify(useCase.addCardUseCase, never()).invoke(invalidCard)
    }
    ...
    fun `test getCardsFromDB returns data`() = runTest {
        ...
        whenever(useCase.getCardUseCase()).thenReturn(flow { emit(mockCards) })
    ...
    fun `test deleteCard is called`() = runTest {
        ...
        whenever(useCase.deleteCardUseCase(addCard)).thenReturn(Unit) // or whatever appropriate return type
        ...
        verify(useCase.deleteCardUseCase).invoke(addCard)
    }
    ...
    fun `test onValidityChange`() {
        ...
        // Asserting that the cardValidity state has been truncated to "12/34" (limit to 5 characters)
